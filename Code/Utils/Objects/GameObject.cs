using MonoFrostTemplate.Code.Utils.Map;
...
namespace MonoFrostTemplate.Code.Utils.Objects
{
    ...
    public class GameObject : IDisposable
    {
        ...
        public virtual void Draw(SpriteBatch spriteBatch)
        {
            ...
            {
                if (game.DebugMode && !(this is Tile) && collider != null && collider.Box.Width > 0)
                {
        ...
        {
            foreach (Hitbox o in scene.GetObjectsOfType<Hitbox>().ToArray())
            {
        ...
        public GameObject[] TouchingObjects(RectangleF collide)
        {
            ...
            if (game.Scene is InGame scene)
            {
                List<GameObject> list = scene.loadedObjects.Where(o => (o.position - position).Length() < 320).ToList();
                list.Remove(this);
                foreach (GameObject obj in list)
                {
        ...
        public virtual void Destroy()
        {
            if (game.Scene is InGame scene)
            {
                scene.loadedObjects.Remove(this);
